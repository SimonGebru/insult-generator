...
function App() {
  ...
  const addInsult = (newInsult) => {
    ...
    console.log("Ny förolämpning tillagd:", newInsult);
  };
  ...
  return (
    <div className="app">
      <h1>Shakespeare Insults Generator</h1>
      <ShowInsult insult={randomInsult()} />

      <h2>Alla förolämpningar:</h2>
      <div className="insults-list">
        {insults.map((item, index) => (
          <ShowInsult key={index} insult={item} />
        ))}
      </div>

      <AddInsult onAddInsult={addInsult} />
    </div>
  );
}
